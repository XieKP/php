...
RUN set -eux; \
	...
	if [ -d php ];then \
    	 ...
    	 sed -i "s|;date.timezone =.*|date.timezone = ${TIMEZONE}|"  php/php.ini; \
         sed -i "s|memory_limit =.*|memory_limit = ${PHP_MEMORY_LIMIT}|" php/php.ini; \
         sed -i "s|upload_max_filesize =.*|upload_max_filesize = ${MAX_UPLOAD}|" php/php.ini; \
         sed -i "s|max_file_uploads =.*|max_file_uploads = ${PHP_MAX_FILE_UPLOAD}|" php/php.ini;  \
         sed -i "s|post_max_size =.*|post_max_size = ${PHP_MAX_POST}|" php/php.ini; \
         sed -i "s/;cgi.fix_pathinfo=1/cgi.fix_pathinfo=0/" php/php.ini; \
	fi; \
	if [ -d php-fpm.d ]; then \
